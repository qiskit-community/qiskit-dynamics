...
def diffrax_dopri5(
    ...
    **kwargs,
    ...
    dt = np.abs(t_list[-1] - t_list[0]) / len(t_list)

    term = ODETerm(lambda y, t, _: rhs(np.real(t_direction * t), y) * t_direction)
    solver = Dopri5()

    results = diffeqsolve(
        term, solver, t0=t_list[0], t1=t_list[1], dt0=dt, y0=Array(y0, dtype=complex), **kwargs
    )
