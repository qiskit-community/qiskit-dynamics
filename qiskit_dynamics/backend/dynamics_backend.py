        ...
        * If a sparse evaluation mode is chosen, the rotating frame will be set to the diagonal of
        ...
        Args:
            backend: The ``Backend`` instance to build the :class:`.DynamicsBackend` from.
               Note that while the type hint indicates that `backend` should be a :class:`~qiskit.providers.backend.BackendV1` instance,
               this method also works for :class:`~qiskit.providers.backend.BackendV2` instances that have been set up with sufficiently populated ``configuration`` and ``defaults`` for backwards compatibility.
