...
class DynamicalOperator(ABC):
	...
	def s_type(self, s_type):
		"""A setter of the operator's type."""
		s_type = s_type.lower()
		self._s_type = s_type
	...
	def get_operator_matrix(self, dim: int) -> Any:
		...
		"""
		raise Exception(f"Operator type `{self.s_type}` unknown or unsupported.")
...
class OperatorBuilder(ABC):
	...
	def build_dictionaries(self, operators: Union[DynamicalOperator, List[DynamicalOperator]],
		...
		Args:
			operators: An DynamicalOperator or a list of DynamicalOperators, for each one the return
		...
		operators_repo = dict()
		for op in operators:
			results.append(self._build_one_dict(op, operators_repo, prune_subsystems))
		if b_flatten:
			results = results[0]
		return results, operators_repo

	def _build_one_dict(self, operator: DynamicalOperator, operators_repo: dict,
		...
		if operator.compound_type == '+':  # The sub-tree root is a sum of two operators
			...
			for op in operator.compound_ops:
				...
				op_dict: dict = self._build_one_dict(op, operators_repo, prune_subsystems)
			...
			new_val = complex(1.)
			for op in operator.compound_ops:
				op_dict = self._build_one_dict(op, operators_repo, prune_subsystems)
			...
			val = complex(1.)
			if prune_subsystems is not None and operator.system_id in prune_subsystems:
				result = dict()
			else:
				operators_repo[DynamicalOperatorKey(operator)] = operator
				# TODO Note that there's no way to verify uniqueness of the implementing operator
				result = {tuple([DynamicalOperatorKey(operator)]): val}
		else:
	...
	def build_matrices(self, operators: Union[DynamicalOperator, Dict, List[DynamicalOperator], List[Dict]],
		...
		Raises:
			An exception if an unidentified operation was found in the tree.
		"""
		# TODO implement "pruning" (removal of subsystem_dims)
		...
		if id_matrix_op is None:
			id_matrix_op = Id()
		self._null_matrix_op = null_matrix_op
		self._total_dim = 1
		for dim in dims:
			if dim > 0:
				self._total_dim *= dim
		if len(operators) == 0:
			return self._null_matrix_op.get_operator_matrix(self._total_dim)
		...
		for sys_id in subsystem_dims.keys():
			self._sys_ids.append(sys_id)
		self._identity_matrices = []
		if b_dictionaries:
			operators_dict: List[Dict] = operators
		else:
			operators_dict, operators_repo = self.build_dictionaries(operators, prune_subsystems)
		results = []
		for dim in dims:
			self._identity_matrices.append(id_matrix_op.get_operator_matrix(dim))
		for op_dict in operators_dict:
			results.append(self._build_one_matrix(op_dict, operators_repo))
		if b_flatten:
			results = results[0]
		return results

	def _build_one_matrix(self, operator_dict: Dict, operators_repo: Dict) -> np.ndarray:
		matrix = self._null_matrix_op.get_operator_matrix(self._total_dim)
		for key, val in operator_dict.items():
			...
			op_matrix = None
			n_subsystems = len(self._sys_ids)
			for i in range(n_subsystems):
				sub_matrix = sub_matrices.get(self._sys_ids[i], self._identity_matrices[i])
				if i == 0:
					op_matrix = sub_matrix
				else:
					op_matrix = self._null_matrix_op.kron_two_matrices(op_matrix, sub_matrix)
