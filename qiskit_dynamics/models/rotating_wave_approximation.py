...
def rotating_wave_approximation(
    ...
    if model.rotating_frame is None or model.rotating_frame.frame_diag is None:
        ...
        diff_matrix = unp.broadcast_to(diag, (n, n)) - unp.broadcast_to(diag, (n, n)).T
        frame_freqs = diff_matrix.imag / (2 * np.pi)

    if model.rotating_frame.frame_diag is not None:
        frame_shift = unp.diag(model.rotating_frame.frame_diag)
    ...
    if isinstance(model, GeneratorModel):
        ...
        cur_drift = unp.to_dense(model._get_static_operator(True))
        ...
        rwa_operators = get_rwa_operators(
            unp.to_dense(model._get_operators(True)),
            model.signals,
        ...
        cur_drift = unp.to_dense(model._get_static_hamiltonian(True)) + frame_shift
        rwa_drift = cur_drift * (abs(frame_freqs) < cutoff_freq).astype(int)
        rwa_drift = model.rotating_frame.operator_out_of_frame_basis(rwa_drift)

        # static dissipator part
        cur_static_dis = unp.to_dense(model._get_static_dissipators(in_frame_basis=True))
        rwa_static_dis = None
        ...
        cur_ham_ops = unp.to_dense(model._get_hamiltonian_operators(in_frame_basis=True))
        cur_dis_ops = unp.to_dense(model._get_dissipator_operators(in_frame_basis=True))
...
def get_rwa_operators(
    ...
    frame_freqs = unp.broadcast_to(frame_freqs, (num_components, n, n))
    carrier_freqs = carrier_freqs.reshape((num_components, 1, 1))

    pos_pass = unp.abs(carrier_freqs + frame_freqs) < cutoff_freq
    pos_terms = current_ops * pos_pass.astype(int)  # G_i^+

    neg_pass = unp.abs(-carrier_freqs + frame_freqs) < cutoff_freq
    ...
    return rotating_frame.operator_out_of_frame_basis(
        _numpy_multi_dispatch(real_component, imag_component, path="append", axis=0)
    )
