...
def jax_lanczos_basis(array: np.ndarray, v_0: np.ndarray, k_dim: int):
    ...
    #     x[idx] = y  -->  x = x.at[idx].set(y)
    beta = beta.at[0].set((np.sqrt(np.abs(projection.conj().T @ projection)))[0, 0])
    ...
    for i in range(1, k_dim, 1):
        ...
        # x[idx] = y  -->  x = x.at[idx].set(y)
        beta = beta.at[i].set(np.sqrt(np.abs(projection.conj().T @ projection)))
...
def jax_lanczos_eig(array: np.ndarray, v_0: np.ndarray, k_dim: int):
    """
