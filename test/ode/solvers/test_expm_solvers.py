...
from qiskit_ode.solvers import scipy_expm_solver, jax_expm_solver
...
class TestExpmSolver(QiskitOdeTestCase):
    """Test cases for scipy_expm_solver."""

    def setUp(self):
        # some generators for testing
        self.constant_generator = lambda t: -1j * np.array([[0., 1.], [1., 0.]])
        self.linear_generator = lambda t: -1j * np.array([[0., 1. - 1j * t], [1. + 1j * t, 0.]])
    ...
    def test_t_eval_arg_no_overlap(self):
        ...
        results = self.expm_solver(self.constant_generator,
                                   t_span,
                                   y0,
                                   max_dt=0.1,
                                   t_eval=t_eval)
    ...
    def test_t_eval_arg_overlap(self):
        ...
        gen = np.array(self.constant_generator(0.))

        expected_y = np.array([expm(0. * gen), expm(0.5 * gen), expm(0.78 * gen)])
    ...
    def test_t_eval_arg_overlap_backwards(self):
        ...
        expected_y = np.array([expm(0.78 * gen), expm(0.5 * gen), expm(0. * gen)])
    ...
    def test_solve_linear_generator(self):
        ...
        results = self.expm_solver(self.linear_generator,
                                   t_span,
                                   y0,
                                   max_dt=0.1,
                                   t_eval=t_eval)
        ...
        dt2 = (0.78 - 0.5) / 3
        expected_y2 = (expm(dt2 * gen(0.5 + 2.5 * dt2)) @
    ...
    def test_solve_linear_generator_backwards(self):
        ...
        dt0 = (1. - 0.78) / 3
        expected_y0 = (expm(- dt0 * gen(1. - 2.5 * dt0)) @
                       expm(- dt0 * gen(1. - 1.5 * dt0)) @
                       expm(- dt0 * gen(1. - 0.5 * dt0)))
        dt1 = (0.78 - 0.5) / 3
        expected_y1 = (expm(- dt1 * gen(0.78 - 2.5 * dt1)) @
                       expm(- dt1 * gen(0.78 - 1.5 * dt1)) @
                       expm(- dt1 * gen(0.78 - 0.5 * dt1)) @
                       expected_y0)
        expected_y2 = (expm(- 0.1 * gen(0.5 - 1.5 * 0.1)) @
                       expm(- 0.1 * gen(0.5 - 0.5 * 0.1)) @
                       expected_y1)
